...
const FormatStep = () => {
  ...
  const softcoverImage = "data:image/jpeg;base64,";
      ...
      try {
        ...
        const startBookGeneration = async () => {
          try {
            ...
            // 更新书籍内容到数据库
            const bookContent = (contentResult as any).bookContent;
            if (bookContent) {
              await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/update-book-data`, {
                method: 'POST',
                body: JSON.stringify({ 
                  orderId, 
                  bookContent
            ...
            // 更新内页PDF到数据库
            const interiorPdf = (interiorResult as any).pdfOutput || (interiorResult as any).pdf;
            if (interiorPdf) {
              await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/update-book-data`, {
                method: 'POST',
                body: JSON.stringify({ 
            ...
            // 更新封面PDF到数据库
            const coverPdf = (coverResult as any).pdfOutput || (coverResult as any).pdf;
            if (coverPdf) {
              await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/update-book-data`, {
                method: 'POST',
                body: JSON.stringify({ 
